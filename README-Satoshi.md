## Mottu Rentals – Visão Técnica (Edição Satoshi)
...
- Banco: `ConnectionStrings:Postgres` ou `MOTTU_POSTGRES_CONNECTION`.
...
## Banco e Migrações
- Os scripts definem `MOTTU_POSTGRES_CONNECTION` se não informado.
- Migrações são aplicadas automaticamente pelos scripts e nos testes de integração.
- Comandos manuais:
```powershell
dotnet tool restore
...
- "relation ... does not exist": aplique migrações (os scripts fazem isso). Verifique `MOTTU_POSTGRES_CONNECTION`.
...
## Smoke Test Rápido (modo Docker)
1) Inicie com o script e crie uma moto:
```bash
curl -s -X POST http://localhost:5000/motorcycles \
  -H "Content-Type: application/json" \
  -d '{"identifier":"mc-001","year":2024,"model":"ModelX","plate":"ABC1A23"}'
```
2) Verifique no banco (dentro do container):
```bash
docker exec -it mottu_postgres psql -U postgres -d mottu_rentals -c "select identifier, year, model, plate from motorcycles;"
```
